...
const CalenderCS = () => {
    ...
    useEffect(() => {
        const fetchBookedSlots = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/api/booked-slots/development`);
                                ...
                                <div className="time-slots">
                                    {timeSlots.map((time) => {
                                        ...
                                        return (
                                            <button
                                                key={time}
                                                className={`time-slot ${selectedTime === time ? 'selected-time' : ''} ${isBooked ? 'disabled-time' : ''}`} // Disable booked time slots
                                                onClick={() => !isBooked && handleTimeSelect(time)} // Prevent selecting booked times
